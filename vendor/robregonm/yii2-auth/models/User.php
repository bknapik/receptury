...
class User extends ActiveRecord implements IdentityInterface
{
	...
	private $statuses = [
		...
		self::STATUS_SUSPENDED => 'Zawieszony',
	];
 ...
 */
    ...
    /**
     * @return int|string current user username
     */
	...
	public function attributeLabels()
	{
		return [
			'id' => 'ID',
			'username' => Yii::t('auth.user', 'Użytkownik'),
			'email' => Yii::t('auth.user', 'Email'),
			'password' => Yii::t('auth.user', 'Hasło'),
			'password_hash' => Yii::t('auth.user', 'Skrót hasła'),
			'password_reset_token' => Yii::t('auth.user', 'Token resetujący hasło'),
			'auth_key' => Yii::t('auth.user', 'Klucz autentykacji'),
			'status' => Yii::t('auth.user', 'Status'),
			'last_visit_time' => Yii::t('auth.user', 'Czas ostatniej wizyty'),
			'create_time' => Yii::t('auth.user', 'Czas utworzenia'),
			'update_time' => Yii::t('auth.user', 'Czas aktualizacji'),
			'delete_time' => Yii::t('auth.user', 'Czas usunięcia'),
		];
	}
	...
	public function beforeSave($insert)
	{
		if (parent::beforeSave($insert)) {
			if (($this->isNewRecord || in_array($this->getScenario(), ['resetPassword', 'profile'])) && !empty($_POST['User']['password'])) {
				$this->password_hash = Yii::$app->getSecurity()->generatePasswordHash($_POST['User']['password']);
			}
			if ($this->isNewRecord) {
